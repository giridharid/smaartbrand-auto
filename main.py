...
async def get_drivers(
    ...
    agg = df.groupby('aspect').agg({
        'sentiment': [
            lambda x: (x == 1).sum(),  # positive_count
            lambda x: (x == -1).sum(), # negative_count
...
async def compare_items(
    ...
    for item in item_list:
        ...
        # Aggregate
        agg = item_df.groupby('aspect').agg({
            'sentiment': [
                lambda x: (x == 1).sum(),
                lambda x: (x == -1).sum(),
                'count'
...
async def get_sample_reviews(
    ...
    for _, row in df.iterrows():
        reviews.append({
            ...
            "sentiment": "positive" if row.get('masi_overall') == 1 else "negative" if row.get('masi_overall') == -1 else "neutral",
            "brand": row.get('brand', ''),
            "model": row.get('model', ''),
...
def gather_context_data_cached(intent: dict) -> dict:
    ...
    try:
        ...
        if intent.get("compare") or not brand:
            comp_agg = df.groupby('brand').agg({
                'sentiment': [
                    lambda x: (x == 1).sum(),
                    lambda x: (x == -1).sum()
                ]
            }).reset_index()
            comp_agg.columns = ['brand', 'positive', 'negative']
            comp_agg['satisfaction'] = (comp_agg['positive'] * 100.0 / 
                (comp_agg['positive'] + comp_agg['negative']).replace(0, 1)).round(1)
            comp_agg = comp_agg.sort_values(comp_agg['positive'] + comp_agg['negative'], ascending=False).head(10)
